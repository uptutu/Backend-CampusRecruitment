...
如果一棵 <u>完全二叉树</u> 的任意一个非终端结点的元素都不小于其左儿子结点和右儿子结点（如果有的话） 的元素，则称此完全二叉树为最大堆。

同样，如果一棵完全二叉树的任意一个非终端结点的元素都不大于其左儿子结点和右儿子结点（如果 有的话）的元素，则称此完全二叉树为最小堆。

概念上是一个颗二叉树，但是实际上可以用一个定长度的数组实现。一个 index 中的父几点为 (index-1)/2 取整，左孩子为 index*2+1 右孩子为 index*2 + 2 
