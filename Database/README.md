...
`表结构` 就是该表的名称是啥，表里边有多少捏，每列的数据类型是啥，有啥约束条件和索引，用的是啥字符集和比较规则等等信息，这些信息都体现在了我们的建表语句中。为了保存这些信息，`InnoDB`和`MyISAM`这两种存储引擎都在`数据目录`下对应的数据库子目录下创建了一个专门用于描述表结构的文件，文件名是这样：
...
![https://user-gold-cdn.xitu.io/2019/5/3/16a7b843e05c8e33?imageslim](README.assets/16a7b843e05c8e33.png)
